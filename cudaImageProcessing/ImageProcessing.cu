...
#include <string>
#include <cstring>
__global__ void GBlur(unsigned char* Input, unsigned char* Output, int numRows, int numCols, float* filter, int Width)
{
...
__global__ void divideChannels( uchar3* RGB, int numRows, int numCols, unsigned char* R, unsigned char* G, unsigned char* B)
{
    int cols = blockIdx.x * blockDim.x + threadIdx.x;
    int rows = blockIdx.y * blockDim.y + threadIdx.y;
    if (cols >= numCols || rows >= numRows)
...
__global__ void combineChannels( unsigned char*  R,  unsigned char*  G,  unsigned char*  B, uchar3*  RGB, int numRows, int numCols)
{
    int cols = blockIdx.x * blockDim.x + threadIdx.x;
    int rows = blockIdx.y * blockDim.y + threadIdx.y;
    if (cols >= numCols || rows >= numRows)
...
extern "CUDA"
{
    void blur(uchar3 * inputRGB, uchar3* outputRGB, int numRows, int numCols)
    {
        unsigned char *host_RED, *host_GREEN, *host_BLUE, *RED, *GREEN, *BLUE;
        unsigned char *R, *G, *B;
	    ...
	    Malloc((void**)& host_RED, N * M * sizeof(unsigned char));
	    cudaMalloc((void**)& RED, N * M * sizeof(unsigned char));
	    checkCudaErr("");
	    cudaMalloc((void**)& R, N * M * sizeof(unsigned char));
	    checkCudaErr("");
	    Malloc((void**)& host_GREEN, N * M * sizeof(unsigned char));
	    cudaMalloc((void**)& GREEN, N * M * sizeof(unsigned char));
	    checkCudaErr("");
	    cudaMalloc((void**)& R, N * M * sizeof(unsigned char));
	    checkCudaErr("");
	    Malloc((void**)& host_BLUE, N * M * sizeof(unsigned char));
	    cudaMalloc((void**)& BLUE, N * M * sizeof(unsigned char));
	    checkCudaErr("");
	    cudaMalloc((void**)& R, N * M * sizeof(unsigned char));
	    checkCudaErr("");
